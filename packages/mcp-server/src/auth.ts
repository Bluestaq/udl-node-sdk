...
export const parseClientAuthHeaders = (req: IncomingMessage, required?: boolean): Partial<ClientOptions> => { if (req.headers.authorization) {
  ...
  switch (scheme) {
    case 'Basic':
     ...
     return { username: rawValue.slice(0, rawValue.search(':')), password: rawValue.slice(rawValue.search(':') + 1) }
    case 'Bearer':
     return { accessToken: req.headers.authorization.slice("Bearer ".length) };
    default:
      throw new Error('Unsupported authorization scheme. Expected the "Authorization" header to be a supported scheme (Basic, Bearer).');
  }
} else if (required) {
  throw new Error('Missing required Authorization header; see WWW-Authenticate header for details.');
}

const username = Array.isArray(req.headers['x-udl-auth-username']) ? req.headers['x-udl-auth-username'][0] : req.headers['x-udl-auth-username']
const password = Array.isArray(req.headers['x-udl-auth-password']) ? req.headers['x-udl-auth-password'][0] : req.headers['x-udl-auth-password']
const accessToken = Array.isArray(req.headers['x-udl-access-token']) ? req.headers['x-udl-access-token'][0] : req.headers['x-udl-access-token']
