...
export class History extends APIResource {
   ...
   * for await (const weatherReportFull of client.weatherReport.history.list(
   ...
   * ```
   */
  list(query: HistoryListParams, options?: RequestOptions): PagePromise<WeatherReportFullsOffsetPage, WeatherReportFull> {
    return this._client.getAPIList('/udl/weatherreport/history', OffsetPage<WeatherReportFull>, { query, ...options });
  }
   ...
   * @example
   * ```ts
   * await client.weatherReport.history.aodr({
   *   obTime: '2019-12-27T18:11:19.117Z',
   * });
   * ```
   */
  aodr(query: HistoryAodrParams, options?: RequestOptions): APIPromise<void> {
    return this._client.get('/udl/weatherreport/history/aodr', { query, ...options, headers: buildHeaders([{Accept: '*/*'}, options?.headers]) });
  }
  ...
  count(query: HistoryCountParams, options?: RequestOptions): APIPromise<string> {
    return this._client.get('/udl/weatherreport/history/count', { query, ...options, headers: buildHeaders([{Accept: 'text/plain'}, options?.headers]) });
  }
}
 ...
 */
export interface WeatherReportFull {
  /**
