...
export interface EquipmentUpdateParams {
   ...
   * political entity to which the equipment geographic coordinates reside. This
  ...
  /**
   * Body param: WGS84 longitude of the location, in degrees. -180 to 180 degrees
  ...
  /**
   * Body param: Air Defense District (ADD) or Air Defense Area (ADA) in which the
   * geographic coordinates reside.
   */
  airDefArea?: string;

  /**
   * Body param: The DoD Standard Country Code designator for the country or
   * political entity to which the equipment owes its allegiance. This field will be
   * set to "OTHR" if the source value does not match a UDL Country code value
   * (ISO-3166-ALPHA-2).
   */
  allegiance?: string;

  /**
   * Body param: Specifies an alternate allegiance code if the data provider code is
   * not part of an official Country Code standard such as ISO-3166 or FIPS. This
   * field will be set to the value provided by the source and should be used for all
   * Queries specifying allegiance.
   */
  altAllegiance?: string;

  /**
   * Body param: Specifies an alternate country code if the data provider code is not
   * part of an official Country Code standard such as ISO-3166 or FIPS. This field
   * will be set to the value provided by the source and should be used for all
   * Queries specifying a Country Code.
   */
  altCountryCode?: string;

  /**
   * Body param: Unique identifier of the Equipment record from the originating
   * system.
   */
  altEqpId?: string;

  /**
   * Body param: Indicates the importance of the equipment. Referenced, but not
   * constrained to, the following class ratings type classifications.
   *
   * 0 - Not of significant importance of the system
   *
   * 1 - Primary importance to system
  ...
  /**
   * Body param: The physical manner of being or state of existence of the entity. A
  ...
  /**
   * Body param: Availability of the entity relative to its condition. Indicates the
   * reason the entity is not fully operational. The specific usage and enumerations
   * contained in this field may be found in the documentation provided in the
   * referenceDoc field. If referenceDoc not provided, users may consult the data
   * provider.
   */
  ...
  /**
   * Body param: Indicates any of the magnitudes that serve to define the position of
   * a point by reference to a fixed figure, system of lines, etc. specified in
  ...
  /**
   * Body param: A mathematical model of the earth used to calculate coordinates on a
   * map. US Forces use the World Geodetic System 1984 (WGS 84), but also use maps by
  ...
  /**
   * Body param: Indicates the plus or minus error assessed against the method used
   ...
   * referenced to (above or below) Mean Sea Level (MSL) vertical datum.
   */
  elevMsl?: number;

  /**
   * Body param: Indicates the confidence level expressed as a percent that a
   * specific geometric spatial element, ELEVATION_MSL linear accuracy, has been
   * vertically positioned to within a specified vertical accuracy. Confidence level
   * has a range of values from 0 to 100, with 100 being highest level of confidence.
   */
  elevMslConfLvl?: number;

  /**
   * Body param: Indicates the plus or minus error assessed against the method used
   * to derive the elevation, measured in meters.
   */
  elevMslDerivAcc?: number;

  /**
   * Body param: Designated equipment code assigned to the item of equipment or an
   * abbreviation record type unique identifier. Users should consult the data
   * provider for information on the equipment code structure.
   */
  eqpCode?: string;

  /**
   * Body param: Uniquely identifies each item or group of equipment associated with
   * a unit, facility or site.
   */
  eqpIdNum?: string;

  /**
   * Body param: Eval represents the Intelligence Confidence Level or the
   * Reliability/degree of confidence that the analyst has assigned to the data
   * within this record. The numerical range is from 1 to 9 with 1 representing the
   * highest confidence level.
   */
  eval?: number;

  /**
   * Body param: Functional Production Area (FPA) under the Shared Production Program
   * (SPP). Producers are defined per country per FPA. The specific usage and
   * enumerations contained in this field may be found in the documentation provided
   * in the referenceDoc field. If referenceDoc not provided, users may consult the
   * data provider.
   */
  ...
  /**
   * Body param: Indicates the function or mission of this equipment, which may or
   * may not be engaged in at any particular time. Typically refers to a unit,
   * organization, or installation/facility performing a specific function or mission
   * such as a redistribution center or naval shipyard. The specific usage and
   * enumerations contained in this field may be found in the documentation provided
   * in the referenceDoc field. If referenceDoc not provided, users may consult the
   * data provider.
   */
  ...
  /**
   * Body param: Principal operational function being performed. The specific usage
   * and enumerations contained in this field may be found in the documentation
   * provided in the referenceDoc field. If referenceDoc not provided, users may
   * consult the data provider.
   */
  ...
  /**
   * Body param: The distance between Mean Sea Level and a referenced ellipsoid,
   * measured in meters.
   */
  geoidalMslSep?: number;

  /**
   * Body param: Estimated identity of the Site (ASSUMED FRIEND, FRIEND, HOSTILE,
   * FAKER, JOKER, NEUTRAL, PENDING, SUSPECT, UNKNOWN):
   *
   * ASSUMED FRIEND: Track assumed to be a friend due to the object characteristics,
   * behavior, and/or origin.
   *
  ...
  /**
   * Body param: Unique identifier of the operating unit associated with the
   * equipment record.
   */
  idOperatingUnit?: string;

  /**
   * Body param: Unique identifier of the Parent equipment record associated with
   * this equipment record.
   */
  idParentEquipment?: string;

  /**
   * Body param: Unique identifier of the Site Entity associated with the equipment
   * record.
   */
  idSite?: string;

  /**
   * Body param: Indicates the reason that the equipment is at that location. The
   * specific usage and enumerations contained in this field may be found in the
   * documentation provided in the referenceDoc field. If referenceDoc not provided,
   * users may consult the data provider.
   */
  ...
  /**
   * Body param: The Military Grid Reference System is the geocoordinate standard
   * used by NATO militaries for locating points on Earth. The MGRS is derived from
   * the Universal Transverse Mercator (UTM) grid system and the Universal Polar
   * Stereographic (UPS) grid system, but uses a different labeling convention. The
   * MGRS is used as geocode for the entire Earth. Example of an milgrid coordinate,
   * or grid reference, would be 4QFJ12345678, which consists of three parts:
   *
   * 4Q (grid zone designator, GZD)
   *
   * FJ (the 100,000-meter square identifier)
  ...
  /**
   * Body param: Indicates the grid system used in the development of the milGrid
  ...
  /**
   * Body param: Generic type this specific piece of equipment belongs to, and the
   * identifying nomenclature which describes the equipment.
   */
  nomen?: string;

  /**
   * Body param: Internationally recognized water area in which the vessel is most
   * likely to be deployed or in which it normally operates most frequently.
   */
  operAreaPrimary?: string;

  /**
   * Body param: The Degree to which an entity is ready to perform the overall
  ...
  /**
   * Body param: Political subdivision in which the geographic coordinates reside.
  ...
  /**
   * Body param: Relative to the parent entity, the total number of military
   * personnel or equipment assessed to be on-hand (OH).
   */
  qtyOH?: number;

  /**
   * Body param: Validity and currency of the data in the record to be used in
   * conjunction with the other elements in the record as defined by SOPs.
   * Referenced, but not constrained to, the following record status type
   * classifications.
   *
   * A - Active
   *
  ...
  /**
   * Body param: The reference documentiation that specifies the usage and
   * enumerations contained in this record. If referenceDoc not provided, users may
   * consult the data provider.
   */
  referenceDoc?: string;

  /**
   * Body param: Responsible Producer - Organization that is responsible for the
   * maintenance of the record.
   */
  resProd?: string;

  /**
   * Body param: Date on which the data in the record was last reviewed by the
   * responsible analyst for accuracy and currency. This date cannot be greater than
   * the current date.
   */
  reviewDate?: string;

  /**
   * Body param: Provider specific sequential number assigned to the equipment.
   */
  seqNum?: number;

  /**
   * Body param: Array of UUID(s) of the UDL data record(s) that are related to this
   * equipment record. See the associated 'srcTyps' array for the specific types of
   * data, positionally corresponding to the UUIDs in this array. The 'srcTyps' and
   * 'srcIds' arrays must match in size. See the corresponding srcTyps array element
   * for the data type of the UUID and use the appropriate API operation to retrieve
   * that object.
   */
  ...
  /**
   * Body param: Array of UDL record types such as AIRCRAFT, VESSEL, EO, MTI that are
   * related to this equipment record. See the associated 'srcIds' array for the
  ...
  /**
   * Body param: A standard scheme for symbol coding enabling the transfer, display
   * and use of symbols and graphics among information systems, as per MIL-STD 2525B,
   * and supported by the element AFFILIATION.
   */
  symCode?: string;

  /**
   * Body param: Universal Transverse Mercator (UTM) grid coordinates.
   *
   * Pos. 1-2, UTM Zone Column [01-60
   *
   * Pos. 3, UTM Zone Row [C-HJ-NP-X]
  ...
  /**
   * Body param: World Aeronautical Chart identifier for the area in which a
   * designated place is located.
   */
  wac?: string;
}
