...
export class Item extends APIResource {
   ...
   */
  update(params: ItemUpdateParams, options?: Core.RequestOptions): Core.APIPromise<void> {
    const { path_id, body_id, ...body } = params;
    return this._client.put(`/udl/item/${path_id}`, {
...
export interface ItemUpdateParams {
  ...
  /**
   * Body param: The tracking identifier of an item or person. May be similar in
   * representation of a barcode or UPC. If no scanCode or tracking number equivalent
   * is available, 'NONE' should be used.
   */
  scanCode: string;
  ...
  /**
   * Body param: The item type of this record (e.g. EQUIPMENT, CARGO, PASSENGER).
   */
  type: string;
  ...
  /**
   * Body param: Array of keys that may be associated to the accepting system data.
  ...
  /**
   * Body param: Additional data required to find this item in the accepting system.
   */
  accSysNotes?: string;

  /**
   * Body param: Name of the system that accepted this item from a customer. Where a
   * user or application could go look for additional information.
   */
  accSystem?: string;

  /**
   * Body param: Array of values for the keys that may be associated to the accepting
   * system data. The entries in this array must correspond to the position index in
   * accSysKeys array. This array must be the same length as accSysKeys.
   */
  accSysValues?: Array<string>;

  /**
   * Body param: Flag indicating this item is planned to be airdropped. Applicable
   * for cargo and passenger item types only.
   */
  airdrop?: boolean;

  /**
   * Body param: Name of the additional data format so downstream consuming
   * applications can know how to parse it. Typically includes the source system name
   * and the format name.
   */
  altDataFormat?: string;

  /**
   * Body param: The type of cargo (e.g. PALLET, ROLLING STOCK, LOOSE, OTHER).
   * Applicable for cargo item types only.
   */
  cargoType?: string;

  /**
   * Body param: How far left or right of centerline is the item in meters.
   * Applicable for cargo and passenger item types only.
   */
  centerlineOffset?: number;

  /**
   * Body param: Center of gravity position of the item, measured from the item's
   * front datum, in centimeters.
   */
  cg?: number;

  /**
   * Body param: The classification code of the commodity or group of commodities.
   */
  commodityCode?: string;

  /**
   * Body param: The classification system denoting the commodity code, commodityCode
   * (e.g. AIR, WATER, NMFC, UFC, STCC, DODUNQ, etc.).
   */
  commoditySys?: string;

  /**
   * Body param: Flag indicating this item acts as a container and contains
   * additional items.
   */
  container?: boolean;

  /**
   * Body param: The departure code or location where this item has left or is
   * leaving.
   */
  departure?: string;

  /**
   * Body param: The destination of the item, typically an ICAO or port code.
   * Applicable for cargo and passenger item types only.
   */
  destination?: string;

  /**
   * Body param: United States Distinguished Visitor Code, only applicable to people.
   */
  dvCode?: string;

  /**
   * Body param: The fuselage station of the item measured from the reference datum,
   * in centimeters. Applicable for cargo and passenger item types only.
   */
  fs?: number;

  /**
   * Body param: Array of UN hazard classes or division codes that apply to this
   * item.
   */
  hazCodes?: Array<number>;

  /**
   * Body param: Height of the cargo in meters. Applicable for cargo item types only.
   */
  height?: number;

  /**
   * Body param: The UDL ID of the air load plan this item is associated with.
   */
  idAirLoadPlan?: string;

  /**
   * Body param: Array of tracking identifiers that are contained within this item.
   */
  itemContains?: Array<string>;

  /**
   * Body param: Array of keys that may be associated to this item. The entries in
  ...
  /**
   * Body param: The latest acceptable arrival date of the item at the destination,
   * in ISO 8601 date-only format (e.g. YYYY-MM-DD).
   */
  lastArrDate?: string;

  /**
   * Body param: Length of the cargo in meters. Applicable for cargo item types only.
   */
  length?: number;

  /**
   * Body param: Moment of the item in Newton-meters. Applicable for equipment and
   * cargo item types only.
   */
  moment?: number;

  /**
   * Body param: Name of the item. Applicable for equipment and cargo item types
   * only.
   */
  name?: string;

  /**
   * Body param: Net explosive weight of the item, in kilograms.
   */
  netExpWt?: number;

  /**
   * Body param: Optional notes or comments about this item.
   */
  notes?: string;

  /**
   * Body param: Number of pallet positions or equivalent on the aircraft, ship, or
   * conveyance equipment that this item occupies.
   */
  numPalletPos?: number;
  ...
  /**
   * Body param: The code denoting the type of material item.
   */
  productCode?: string;

  /**
   * Body param: The assigning system that denotes the type of material item,
   * productCode (e.g. NSN-national stock number, NDC-national drug code,
   * MPN-manufacturer part number, etc.).
   */
  productSys?: string;

  /**
   * Body param: The military branch receiving this item.
   */
  receivingBranch?: string;

  /**
   * Body param: The name of the unit receiving this item.
   */
  receivingUnit?: string;

  /**
   * Body param: The algorithm name or standard that generated the scanCode (e.g.
   * UPC-A, EAN-13, GTIN, SSCC, bID, JAN, etc.).
   */
  scGenTool?: string;

  /**
   * Body param: Transportation Control Number of the cargo. Applicable for cargo
   * item types only.
   */
  tcn?: string;

  /**
   * Body param: The unit line number of this item.
   */
  uln?: string;

  /**
   * Body param: Array of values for the keys that may be associated to this tracked
   * item. The entries in this array must correspond to the position index in the
   * keys array. This array must be the same length as keys.
   */
  values?: Array<string>;

  /**
   * Body param: The volume of the item, in cubic meters. Applicable for cargo item
   * types only.
   */
  volume?: number;

  /**
   * Body param: Weight of the item in kilograms (if item is a passenger, include
   * on-person bags).
   */
  weight?: number;

  /**
   * Body param: Timestamp when the weight was taken, in ISO 8601 UTC format with
   * millisecond precision.
   */
  weightTS?: string;

  /**
   * Body param: Width of the cargo in meters. Applicable for cargo item types only.
   */
  width?: number;
}
