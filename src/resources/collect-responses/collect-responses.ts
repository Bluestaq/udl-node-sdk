...
export class CollectResponses extends APIResource {
  ...
  ): Core.APIPromise<Shared.CollectResponseFull> {
    const { path_id, body_id } = params;
    return this._client.get(`/udl/collectresponse/${path_id}`, options);
  }
  ...
  list(
    ...
    return this._client.get('/udl/collectresponse', options);
  }
   ...
   */
  count(params: CollectResponseCountParams, options?: Core.RequestOptions): Core.APIPromise<string> {
    const { createdAt } = params;
...
export interface CollectResponseRetrieveParams {
  ...
  /**
   * Body param: The ID of the Collect Response object to find.
   */
  body_id: string;
}
