...
export class Paths extends APIResource {
   ...
   * @deprecated
   */
  createWithFile(fileContent: string | ArrayBuffer | ArrayBufferView | Blob | DataView, params: PathCreateWithFileParams, options?: RequestOptions): APIPromise<string> {
    const { id, classificationMarking, deleteAfter, description, overwrite, sendNotification, tags } = params
    return this._client.post('/scs/path', { body: fileContent, query: { id, classificationMarking, deleteAfter, description, overwrite, sendNotification, tags }, ...options, headers: buildHeaders([{'Content-Type': 'application/octet-stream'}, options?.headers]) });
  }
}
