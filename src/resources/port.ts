...
export class Port extends APIResource {
   ...
   */
  update(params: PortUpdateParams, options?: Core.RequestOptions): Core.APIPromise<void> {
    const { path_id, body_id, ...body } = params;
    return this._client.put(`/udl/port/${path_id}`, {
...
export interface PortUpdateParams {
  /**
   * Path param: The ID of the Port to update.
   */
  path_id: string;
  ...
  /**
   * Body param: Average time for a vessel at this port in hours.
   */
  avgDuration?: number;

  /**
   * Body param: The country where this port is located.
   */
  countryCode?: string;

  /**
   * Body param: Optional ID from external systems. This field has no meaning within
  ...
  /**
   * Body param: The size of the harbor for this port measured in square kilometers.
   */
  harborSize?: number;

  /**
   * Body param: The type of harbor for this port. The harbor type refers to how a
   * port is physically positioned.
   *
   * COASTAL BREAKWATER (CB)
   *
   * COASTAL NATURAL (CN)
  ...
  /**
   * Body param: Unique identifier of the Site Entity associated with the Port
   * record.
   */
  idSite?: string;
  ...
  /**
   * Body param: The five-character United Nations Code for Trade and Transport
   * Locations (UN/LOCODE) of this port. The first two letters of the code contains
   * the ISO 3166-1 alpha-2 country designation of the port country. The three
   * remaining characters identify a location within that country. Letters are
   * preferred, but if necessary digits 2 through 9 may be used, excluding "0" and
   * "1" to avoid confusion with the letters "O" and "I" respectively.
   */
  locode?: string;
  ...
  /**
   * Body param: Maximum allowed vessel draught. Draught is the principal dimensions
   * of any waterborne vessel defined as the distance between the ship’s keel and the
   * waterline of the vessel measured in meters.
   */
  maxDraught?: number;
  ...
  /**
   * Body param: Flag indicating whether a pilot is required at this port.
   */
  pilotReqd?: boolean;

  /**
   * Body param: The name of this port.
   */
  portName?: string;

  /**
   * Body param: The shelter afforded from wind, sea, and swell refers to the area
   * where normal port operations are conducted, usually the wharf area. Shelter
   * afforded by the anchorage area may be given for ports where cargo is handled by
   * lighters. Values given are EXCELLENT, FAIR, GOOD, POOR, or NONE.
   */
  shelter?: string;

  /**
   * Body param: The tide range of this port in meters.
   */
  tideRange?: number;
}
