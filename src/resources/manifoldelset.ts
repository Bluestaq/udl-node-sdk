...
export class Manifoldelset extends APIResource {
   ...
   */
  update(params: ManifoldelsetUpdateParams, options?: Core.RequestOptions): Core.APIPromise<void> {
    const { path_id, body_id, ...body } = params;
    return this._client.put(`/udl/manifoldelset/${path_id}`, {
...
export interface ManifoldelsetUpdateParams {
  /**
   * Path param: The ID of the ManifoldElset to update.
   */
  path_id: string;
  ...
  /**
   * Body param: Elset epoch time in ISO 8601 UTC format, with microsecond precision.
   */
  epoch: string;

  /**
   * Body param: Identifier of the parent Manifold record.
   */
  idManifold: string;
  ...
  /**
   * Body param: A placeholder satellite number and not a true NORAD catalog number.
   */
  tmpSatNo: number;
  ...
  /**
   * Body param: The Orbit point furthest from the center of the earth in kilometers.
   */
  apogee?: number;

  /**
   * Body param: The argument of perigee is the angle in degrees formed between the
   * perigee and the ascending node. If the perigee would occur at the ascending
   * node, the argument of perigee would be 0.
   */
  argOfPerigee?: number;

  /**
   * Body param: The drag term for SGP4 orbital model, used for calculating decay
   * constants for altitude, eccentricity etc, measured in inverse earth radii.
   */
  bStar?: number;

  /**
   * Body param: The orbital eccentricity of an astronomical object is a parameter
   * that determines the amount by which its orbit around another body deviates from
   * a perfect circle. A value of 0 is a circular orbit, values between 0 and 1 form
   * an elliptic orbit, 1 is a parabolic escape orbit, and greater than 1 is a
   * hyperbolic escape orbit.
   */
  eccentricity?: number;

  /**
   * Body param: The angle between the equator and the orbit when looking from the
  ...
  /**
   * Body param: Where the satellite is in its orbital path. The mean anomaly ranges
   * from 0 to 360 degrees. The mean anomaly is referenced to the perigee. If the
   * satellite were at the perigee, the mean anomaly would be 0.
   */
  meanAnomaly?: number;

  /**
   * Body param: The constant angular speed required for the body to complete one
   * circular orbit in the same amount of time as the actual elliptical orbit with
   * variable speed. Measured in revolutions per day.
   */
  meanMotion?: number;

  /**
   * Body param: 2nd derivative of the mean motion with respect to time. Units are
   * revolutions per day cubed.
   */
  meanMotionDDot?: number;

  /**
   * Body param: 1st derivative of the mean motion with respect to time. Units are
  ...
  /**
   * Body param: The orbit point nearest to the center of the earth in kilometers.
   */
  perigee?: number;

  /**
   * Body param: Period of the orbit equal to inverse of mean motion.
   */
  period?: number;

  /**
   * Body param: Right ascension of the ascending node, or RAAN is the angle as
   * measured in degrees eastwards (or, as seen from the north, counterclockwise)
  ...
  /**
   * Body param: The current revolution number. The value is incremented when a
   * satellite crosses the equator on an ascending pass.
   */
  revNo?: number;

  /**
   * Body param: The sum of the periapsis and apoapsis distances divided by two. For
   * circular orbits, the semimajor axis is the distance between the centers of the
   * bodies, not the distance of the bodies from the center of mass.
   */
  semiMajorAxis?: number;
}
