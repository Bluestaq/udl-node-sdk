...
export class Seradataopticalpayload extends APIResource {
   ...
   * assistance.
   */
  update(params: SeradataopticalpayloadUpdateParams, options?: Core.RequestOptions): Core.APIPromise<void> {
    const { path_id, body_id, ...body } = params;
    return this._client.put(`/udl/seradataopticalpayload/${path_id}`, {
...
export interface SeradataopticalpayloadUpdateParams {
  /**
   * Path param: The ID of the SeradataOpticalPayload to update.
   */
  path_id: string;
  ...
  /**
   * Body param: Imaging category for this payload, e.g. Multispectral, Infrared,
   * Panchromatic.
   */
  imagingPayloadCategory?: string;
  ...
  /**
   * Body param: Sensor name from Seradata, e.g. TOURNESOL, MESSR (Multispectral
   * Self-Scanning Radiometer), AWFI, etc.
   */
  name?: string;
  ...
  /**
   * Body param: Number of film return canisters.
   */
  numberOfFilmReturnCanisters?: number;
  ...
  /**
   * Body param: Recorder size.
   */
  recorderSize?: string;

  /**
   * Body param: Spectral Band supported by this payload, e.g. Green, Red, Mid-wave
   * infrared, etc.
   */
  spectralBand?: string;

  /**
   * Body param: Frequency limit for this payload, e.g. 0.51 - 0.59.
   */
  spectralFrequencyLimits?: string;
