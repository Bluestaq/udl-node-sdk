...
describe('resource launchDetection', () => {
  ...
  test('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.launchDetection.list({ firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('count: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.launchDetection.count({ firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Unifieddatalibrary.NotFoundError);
  });
  ...
  test('get: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.launchDetection.get('id', { firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Unifieddatalibrary.NotFoundError);
  });
