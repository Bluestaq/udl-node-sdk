...
describe('resource weatherreport', () => {
  ...
  test('list: required and optional params', async () => {
    const response = await client.weatherreport.list({ obTime: '2019-12-27T18:11:19.117Z' });
  });
  ...
  test('count: required and optional params', async () => {
    const response = await client.weatherreport.count({ obTime: '2019-12-27T18:11:19.117Z' });
  });

  test('fileCreate: only required params', async () => {
    const responsePromise = client.weatherreport.fileCreate([
      {
        classificationMarking: 'U',
        dataMode: 'REAL',
        lat: 56.12,
  ...
  test('fileCreate: required and optional params', async () => {
    const response = await client.weatherreport.fileCreate([
      {
        classificationMarking: 'U',
        dataMode: 'REAL',
        lat: 56.12,
  ...
  test('get: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.weatherreport.get('id', { path: '/_stainless_unknown_path' })).rejects.toThrow(
  ...
  test('queryhelp: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.weatherreport.queryhelp({ path: '/_stainless_unknown_path' })).rejects.toThrow(
      Unifieddatalibrary.NotFoundError,
    );
  });
