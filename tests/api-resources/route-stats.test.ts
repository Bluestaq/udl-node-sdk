...
describe('resource routeStats', () => {
  ...
  test('retrieve: request options and params are passed correctly', async () => {
    ...
    await expect(client.routeStats.retrieve('id', { firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('list', async () => {
    const responsePromise = client.routeStats.list();
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test('list: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.routeStats.list({ firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Unifieddatalibrary.NotFoundError);
  });
  ...
  test('count: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.routeStats.count({ firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Unifieddatalibrary.NotFoundError);
  });

  test('createBulk: only required params', async () => {
    const responsePromise = client.routeStats.createBulk({ body: [{
    classificationMarking: 'U',
    dataMode: 'TEST',
    locationEnd: 'KCOS',
    locationStart: 'KDEN',
  ...
  test('createBulk: required and optional params', async () => {
    const response = await client.routeStats.createBulk({ body: [{
    classificationMarking: 'U',
    dataMode: 'TEST',
    locationEnd: 'KCOS',
    locationStart: 'KDEN',
  ...
  test('unvalidatedPublish: only required params', async () => {
    const responsePromise = client.routeStats.unvalidatedPublish({ body: [{
    classificationMarking: 'U',
    dataMode: 'TEST',
    locationEnd: 'KCOS',
    locationStart: 'KDEN',
  ...
  test('unvalidatedPublish: required and optional params', async () => {
    const response = await client.routeStats.unvalidatedPublish({ body: [{
    classificationMarking: 'U',
    dataMode: 'TEST',
    locationEnd: 'KCOS',
    locationStart: 'KDEN',
