...
describe('resource ephemerisSets', () => {
  ...
  test('retrieve: request options and params are passed correctly', async () => {
    ...
    await expect(client.ephemerisSets.retrieve('id', { firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('list: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.ephemerisSets.list({
    firstResult: 0,
    maxResults: 0,
  ...
  test('count: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.ephemerisSets.count({
    firstResult: 0,
    maxResults: 0,
    pointEndTime: '2019-12-27T18:11:19.117Z',
    pointStartTime: '2019-12-27T18:11:19.117Z',
  ...
  test('fileRetrieve: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.ephemerisSets.fileRetrieve('id', { firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Unifieddatalibrary.NotFoundError);
  });
