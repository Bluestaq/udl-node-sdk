...
describe('resource airspaceControlOrders', () => {
  ...
  test('retrieve: request options and params are passed correctly', async () => {
    ...
    await expect(client.airspaceControlOrders.retrieve('id', { firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('list: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.airspaceControlOrders.list({ firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Unifieddatalibrary.NotFoundError);
  });
  ...
  test('count: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.airspaceControlOrders.count({ firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Unifieddatalibrary.NotFoundError);
  });

  test('createBulk: only required params', async () => {
    const responsePromise = client.airspaceControlOrders.createBulk({ body: [{
    classificationMarking: 'U',
    dataMode: 'TEST',
    opExName: 'DESERT WIND',
    originator: 'USCENTCOM',
  ...
  test('createBulk: required and optional params', async () => {
    const response = await client.airspaceControlOrders.createBulk({ body: [{
    classificationMarking: 'U',
    dataMode: 'TEST',
    opExName: 'DESERT WIND',
    originator: 'USCENTCOM',
