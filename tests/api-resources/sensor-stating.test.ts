...
describe('resource sensorStating', () => {
  ...
  test('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.sensorStating.list({ firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('createBulk: only required params', async () => {
    const responsePromise = client.sensorStating.createBulk({ body: [{
    classificationMarking: 'U',
    sensorName: 'SENSOR_NAME',
    source: 'Bluestaq',
  }] });
  ...
  test('createBulk: required and optional params', async () => {
    const response = await client.sensorStating.createBulk({ body: [{
    classificationMarking: 'U',
    sensorName: 'SENSOR_NAME',
    source: 'Bluestaq',
    id: 'ad88770b-d824-443f-bdce-5f9e3fa500a9',
  ...
  test('get: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.sensorStating.get('id', { firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Unifieddatalibrary.NotFoundError);
  });
