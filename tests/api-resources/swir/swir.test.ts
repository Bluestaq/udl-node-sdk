...
describe('resource swir', () => {
  ...
  test('createBulk: only required params', async () => {
    const responsePromise = client.swir.createBulk({ body: [{
    classificationMarking: 'U',
    dataMode: 'TEST',
    source: 'Bluestaq',
    ts: '2021-01-01T01:01:01.123456Z',
  ...
  test('createBulk: required and optional params', async () => {
    const response = await client.swir.createBulk({ body: [{
    classificationMarking: 'U',
    dataMode: 'TEST',
    source: 'Bluestaq',
    ts: '2021-01-01T01:01:01.123456Z',
  ...
  test('get: request options and params are passed correctly', async () => {
    ...
    await expect(client.swir.get('id', { firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
