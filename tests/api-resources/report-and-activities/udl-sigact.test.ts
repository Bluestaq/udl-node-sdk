...
describe('resource udlSigact', () => {
  test('fileGet: request options and params are passed correctly', async () => {
    ...
    await expect(client.reportAndActivities.udlSigact.fileGet('id', { firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Unifieddatalibrary.NotFoundError);
  });

  test('unvalidatedPublish: only required params', async () => {
    const responsePromise = client.reportAndActivities.udlSigact.unvalidatedPublish({ body: [{
  ...
  test('unvalidatedPublish: required and optional params', async () => {
    const response = await client.reportAndActivities.udlSigact.unvalidatedPublish({ body: [{
    classificationMarking: 'U',
    dataMode: 'TEST',
    reportDate: '2018-01-01T16:00:00.123Z',
    source: 'Bluestaq',
