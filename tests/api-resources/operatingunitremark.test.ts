...
describe('resource operatingunitremark', () => {
  ...
  test('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.operatingunitremark.list({ firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('count: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.operatingunitremark.count({ firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Unifieddatalibrary.NotFoundError);
  });

  test('createBulk: only required params', async () => {
    const responsePromise = client.operatingunitremark.createBulk({ body: [{
    classificationMarking: 'U',
    dataMode: 'TEST',
    idOperatingUnit: 'OperatingUnit-ID',
    source: 'some.user',
  ...
  test('createBulk: required and optional params', async () => {
    const response = await client.operatingunitremark.createBulk({ body: [{
    classificationMarking: 'U',
    dataMode: 'TEST',
    idOperatingUnit: 'OperatingUnit-ID',
    source: 'some.user',
  ...
  test('get: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.operatingunitremark.get('id', { firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Unifieddatalibrary.NotFoundError);
  });
