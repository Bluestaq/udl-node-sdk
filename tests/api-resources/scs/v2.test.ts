...
describe('resource v2', () => {
  ...
  test('fileUpload: only required params', async () => {
    const responsePromise = client.scs.v2.fileUpload(await toFile(Buffer.from('Example data'), 'README.md'), { classificationMarking: 'classificationMarking', path: 'path' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test('fileUpload: required and optional params', async () => {
    const response = await client.scs.v2.fileUpload(await toFile(Buffer.from('Example data'), 'README.md'), {
