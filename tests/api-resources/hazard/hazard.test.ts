...
describe('resource hazard', () => {
  ...
  test('createBulk: only required params', async () => {
    const responsePromise = client.hazard.createBulk({ body: [{
    alarms: ['Alarm1', 'Alarm2'],
    alarmValues: [2.7, 2.9],
    classificationMarking: 'U',
    dataMode: 'TEST',
  ...
  test('createBulk: required and optional params', async () => {
    const response = await client.hazard.createBulk({ body: [{
    alarms: ['Alarm1', 'Alarm2'],
    alarmValues: [2.7, 2.9],
    classificationMarking: 'U',
    dataMode: 'TEST',
  ...
  test('get: request options and params are passed correctly', async () => {
    ...
    await expect(client.hazard.get('id', { firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('tuple: only required params', async () => {
    const responsePromise = client.hazard.tuple({ columns: 'columns', detectTime: '2019-12-27T18:11:19.117Z' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
