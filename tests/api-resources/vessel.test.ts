...
describe('resource vessel', () => {
  ...
  test('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.vessel.list({ firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('count: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.vessel.count({ firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Unifieddatalibrary.NotFoundError);
  });

  test('createBulk: only required params', async () => {
    const responsePromise = client.vessel.createBulk({ body: [{
    classificationMarking: 'U',
    dataMode: 'TEST',
    source: 'Bluestaq',
  }] });
  ...
  test('createBulk: required and optional params', async () => {
    const response = await client.vessel.createBulk({ body: [{
    classificationMarking: 'U',
    dataMode: 'TEST',
    source: 'Bluestaq',
    id: '0167f577-e06c-358e-85aa-0a07a730bdd0',
  ...
  test('get: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.vessel.get('id', { firstResult: 0, maxResults: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Unifieddatalibrary.NotFoundError);
  });
