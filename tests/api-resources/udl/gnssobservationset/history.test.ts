...
describe('resource history', () => {
  test('list: only required params', async () => {
    const responsePromise = client.udl.gnssobservationset.history.list({ ts: '2019-12-27T18:11:19.117Z' });
  ...
  test('aodr: only required params', async () => {
    const responsePromise = client.udl.gnssobservationset.history.aodr({ ts: '2019-12-27T18:11:19.117Z' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
