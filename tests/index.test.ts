...
import { APIPromise } from 'unifieddatalibrary/core/api-promise';

import util from 'node:util';
import Unifieddatalibrary from 'unifieddatalibrary';
import { APIUserAbortError } from 'unifieddatalibrary';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['UNIFIEDDATALIBRARY_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Unifieddatalibrary) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['UNIFIEDDATALIBRARY_LOG'] = 'debug';
      const client = new Unifieddatalibrary({
        logger: logger,
        password: 'My Password',
        username: 'My Username',
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['UNIFIEDDATALIBRARY_LOG'] = 'not a log level';
      const client = new Unifieddatalibrary({
        logger: logger,
        password: 'My Password',
        username: 'My Username',
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'UNIFIEDDATALIBRARY_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['UNIFIEDDATALIBRARY_BASE_URL'] = undefined;
    });
